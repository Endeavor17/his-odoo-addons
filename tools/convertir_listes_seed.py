...
CAFE_REGLES = [
    ...
    (
        ...
        r"cure dents|sac |papier aluminium|papier cuissant|paillette)",
        EMBALLAGE,
    ),
    ...
    (
        ...
        r"noyer|nutiss|raya |fruseco|chiwawa)",
        SNACKS,
    ),
    ...
    (
        ...
        r"westmin|vitale|kabir|tabnija|nougat)",
        DIVERS,
    ),
]
...
def construire():
    ...
    for i, row in enumerate(lire("cafeteria_gateaux.csv"), start=1):
        ...
        if code and not code.isdigit():
            ...
            if not re.match(r"^(CAF|COP|RES|NET|SAN)-", code, re.IGNORECASE):
                alertes.append(("cafeteria", designation, "code-barres « %s » non numerique — ignore" % code))
            code = ""
        lignes.append(
            {
                "Nom": designation,
    ...
    for ligne in lignes:
        ...
        if code and code_precedent and code != code_precedent:
            ...
            alertes.append(
                (ligne["Source"], ligne["Nom"], "nom en double, code-barres %s conserve (%s ecarte)" % (garde, jete))
            )
        elif code and not code_precedent:
            precedente["Code_Barres"] = code  # la ligne gardee recupere le code
            alertes.append((ligne["Source"], ligne["Nom"], "doublon fusionne, code-barres %s conserve" % code))
        else:
...
def ecrire(lignes):
    ...
    colonnes = ["Nom", "Categorie_Suggeree", "Type", "Format", "Variante", "Code_Barres", "Source", "Ligne"]
