...
CONDITIONNEMENT = re.compile(r"\b(?:[CFB]\s*/\s*\d+\s*\w*|BT\s*\d+|PQ\s*\d+|PAQ\s*\d+|CT\s*\d+|C/\d+)", re.IGNORECASE)
...
hors = Product.search_count([("categ_id.complete_name", "=like", PREFIXE + "Ménage%")])
