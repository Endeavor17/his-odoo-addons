...
class TestPartnerScope(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.env['ir.config_parameter'].sudo().set_param(
            'point_of_sale.limited_customer_count', 100000,
        )
        cls.config = cls.env['pos.config'].search([], limit=1) \
            or cls.env['pos.config'].create({'name': "Caisse de test"})
    ...
    def _searched(self):
        found = self.env['res.partner'].get_new_partner(
            self.config.id, [('name', 'ilike', "Zineb Candidate")], 0,
        )
    ...
    def test_le_contact_d_un_utilisateur_n_est_jamais_ecarte(self):
        user = self.env['res.users'].create({
            'name': "Zineb Candidate", 'login': 'zineb.pos.scope',
        })
        self.env['hr.applicant'].create({
            'partner_id': user.partner_id.id, 'partner_name': "Zineb Candidate",
        })
