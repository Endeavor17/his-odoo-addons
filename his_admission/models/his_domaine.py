...
class HisDomaine(models.Model):
    ...
    seuil_eligibilite = fields.Float(
        string="Seuil d'eligibilite",
        default=11.0,
        help="Moyenne ponderee en dessous de laquelle le dossier part en verification manuelle.",
    )
    min_bac = fields.Float(
        string="Moyenne BAC minimale",
        default=10.0,
        help="Plancher elimatoire, independant de la moyenne ponderee.",
    )
    min_math = fields.Float(
        string="Note de maths minimale",
        default=0.0,
    ...
    _code_unique = models.Constraint(
        "unique(code)",
        "Ce code de domaine est deja utilise.",
    )
