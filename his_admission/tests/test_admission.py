...
class TestAdmission(TransactionCase):
    ...
    def test_l_encaissement_gagne_le_lead(self):
        ...
        self.assertEqual(
            lead.stage_id,
            self.env.ref("his_crm_pipeline.stage_vente_frais_payes"),
        )
    ...
    def test_la_motivation_vaut_un_point_si_l_une_des_deux_est_remplie(self):
        base = {"specialite_id": self.spec_licence.id, "bac_moyenne": 16.0, "note_math": 16.0}
        self.assertEqual(self._lead_score(**base).score_academique, 9)
        self.assertEqual(
            self._lead_score(motivation_majeure="Passion.", **base).score_academique,
            10,
        )
        self.assertEqual(
            self._lead_score(motivation_his="   ", **base).score_academique,
            9,
    ...
    def test_un_candidat_perdu_avant_l_argent_ne_consomme_aucun_matricule(self):
        ...
        lead = self._lead_avec_dossier()
        personne = lead.his_person_id
        lead.action_set_lost(lost_reason_id=self.env.ref("his_crm_pipeline.lost_reason_trop_cher").id)
    ...
    def _lead_avec_dossier(self, **vals):
        ...
        lead = self.env["crm.lead"].create(
            {
                "name": "Candidature",
                "contact_name": "Nadia Slimani",
                "email_from": "nadia.slimani@example.dz",
                "phone": "0555443322",
                "team_id": self.env.ref("his_crm_pipeline.crm_team_ventes").id,
                "specialite_id": self.spec_licence.id,
                "bac_moyenne": 13.5,
    ...
    def test_la_regle_de_l_admission_part_de_contact_etabli(self):
        ...
        self.assertIn(self.env.ref("his_crm_pipeline.stage_vente_contact_etabli").id, etapes)
        self.assertNotIn(self.env.ref("his_crm_pipeline.stage_vente_pris_en_charge").id, etapes)
    ...
    def test_avec_un_tarif_le_revenu_se_deduit(self):
        ...
        tuile = next(t for t in self._cockpit()["tiles"] if t["cle"] == "revenu_attendu")
