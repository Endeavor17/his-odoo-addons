...
class AcademicEngagement(models.Model):
    ...
    person_id = fields.Many2one("his.person", "Person", required=True, ondelete="cascade", index=True)
    ...
    def _check_process_permission(self, level):
        for engagement in self:
            if engagement.process == "insite":
                self.env["campus.process.permission"]._check_process_permission("insite_engagements", level)
