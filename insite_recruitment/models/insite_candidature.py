...
class InsiteCandidature(models.Model):
    ...
    person_id = fields.Many2one("his.person", "Person", required=True, ondelete="restrict", index=True, tracking=True)
    need_id = fields.Many2one(
        "insite.recruitment.need",
        "Recruitment Need",
        ondelete="set null",
    ...
    raw_submission_ids = fields.One2many("insite.submission", "candidature_id", "Raw Submissions")
    engagement_ids = fields.One2many("academic.engagement", "insite_candidature_id", "Engagements")
    contract_ids = fields.One2many("insite.contract", "candidature_id", "Contracts")
    ...
    @api.depends("contract_ids.state", "contract_ids.create_date", "engagement_ids.state", "engagement_ids.create_date")
    ...
    def _compute_rank_factors(self):
        ...
        specialty_match = bool(
            need
            and need.specialty
            and person.specialite
            and need.specialty.strip().lower() == person.specialite.strip().lower()
        )
        if specialty_match:
            score += 3
        reasons.append(
        ...
        module_experience = bool(
            need and person.insite_engagement_ids.filtered(lambda e: e.module_id == need.module_id)
        )
    ...
    def action_contact(self):
        ...
        for candidature in self:
            if candidature.state != "prospect":
                raise UserError(_("%s is not a prospect, so it cannot be contacted.", candidature.display_name))
            candidature.write({"state": "contacted", "contacted_date": fields.Datetime.now(), "reminder_sent": False})
            if template:
                template.send_mail(candidature.id, force_send=False)
            candidature.message_post(
                body=_("Candidate contacted. The phone call itself is manual — the system only records the outcome.")
            )
        return True
    ...
    def action_reschedule_meeting(self):
        ...
        if not self.meeting_event_id:
            raise UserError(
                _("No meeting is scheduled yet for %s — use 'Schedule Meeting' first.", self.person_id.display_name)
            )
        return {
            "type": "ir.actions.act_window",
    ...
    def _cron_insite_reminder_check(self):
        ...
        validators = (
            self.env["campus.process.permission"]
            .sudo()
            .search(
                [
                    ("process_id.code", "=", "insite_needs"),
                    ("can_validate", "=", True),
                    ("active", "=", True),
                ]
            )
        ...
        for candidature in overdue:
            ...
            for user in validators:
                candidature.activity_schedule(
                    ...
                    note=_(
                        "Contacted on %s with no response yet. Decide whether to wait or "
