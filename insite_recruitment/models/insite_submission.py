...
class InsiteSubmission(models.Model):
    ...
    def _next_reference(self):
        return self.env["ir.sequence"].next_by_code("insite.submission") or fields.Datetime.now().strftime(
            "INS/%Y%m%d/%H%M%S%f"
        )
    ...
    # ------------------------------------------------------------------
    @api.model
    def _insite_identity_matches(
        self, matricule=None, first_name=None, last_name=None, name_ar=None, email=None, phone=None
    ):
    ...
    def _insite_probable_applicants(self, name, name_ar, email, phone):
        ...
        return Applicant.search([("his_person_id", "=", False)] + ["|"] * (len(criteria) - 1) + criteria)
                ...
                person = self.env["his.person"]._insite_create_external(
                    {
                        "name": " ".join(
                            filter(
                                None,
                                [(payload.get("firstName") or "").strip(), (payload.get("lastName") or "").strip()],
                            )
                        ),
    ...
    def _insite_get_or_create_candidature(self, person):
        ...
        self.ensure_one()
        candidature = person.insite_candidature_ids.filtered(lambda c: c.state not in ("declined", "superseded"))[:1]
        if candidature:
            return candidature
