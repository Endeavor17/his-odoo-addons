...
class HisPerson(models.Model):
    ...
    insite_engagement_ids = fields.One2many(
        'academic.engagement', 'person_id', "InSite Engagements")
    insite_engagement_count = fields.Integer(compute='_compute_insite_counts')

    def _compute_insite_counts(self):
        candidatures = dict(self.env['insite.candidature']._read_group(
            [('person_id', 'in', self.ids)], ['person_id'], ['__count']))
        engagements = dict(self.env['academic.engagement']._read_group(
            [('person_id', 'in', self.ids)], ['person_id'], ['__count']))
        for person in self:
    ...
    def _insite_create_external(self, vals):
        ...
        vals.update(type_personne='candidat', source_system='manual',
                    is_internal_teacher='external', match_method='new')
