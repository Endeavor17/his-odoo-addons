...
class InsiteRecruitmentNeed(models.Model):
    ...
    module_id = fields.Many2one("campus.subject", "Module", required=True, ondelete="restrict", index=True)
    ...
    selected_candidature_id = fields.Many2one(
        "insite.candidature", "Selected Candidature", ondelete="set null", copy=False, index="btree_not_null"
    )
    ...
    def _compute_display_name(self):
        for need in self:
            need.display_name = _(
                "%(module)s (%(period)s)",
                module=need.module_id.display_name,
                period=need.academic_period_id.display_name,
            )

    def _compute_module_sheet_ids(self):
        for need in self:
            need.module_sheet_ids = self.env["insite.module.sheet"].search([("engagement_id.need_id", "=", need.id)])
        ...
        if candidature.state not in ("prospect", "declined"):
            raise UserError(
                _(
                    ...
                    candidature=candidature.display_name,
