...
class InsiteContract(models.Model):
    ...
    person_id = fields.Many2one("his.person", "Person", required=True, ondelete="restrict", index=True)
    candidature_id = fields.Many2one("insite.candidature", "Candidature", ondelete="set null", index="btree_not_null")
    ...
    def action_sign(self):
        ...
        for contract in self:
            if contract.state != "accepted":
                raise UserError(
                    _("%s must be accepted by the candidate before it can be signed.", contract.display_name)
                )
            if not contract.document:
                raise UserError(_("Attach the signed contract document before marking it signed."))
