...
class TestInsiteScenarios(InsiteCommon):
    ...
    def test_scenario_a_campus_only(self):
        applicant = self.env["hr.applicant"].create(
            {
                "partner_name": "Karim Campus Only",
                "email_from": "karim.campus.only@example.com",
            }
        )
        self.assertFalse(applicant.his_person_id, "no Person just because a Campus+ applicant exists")
        self.assertEqual(self.Candidature.search_count([("person_id.campus_applicant_ids", "=", applicant.id)]), 0)
    ...
    def test_scenario_b_insite_only(self):
        ...
        submission = self._make_submission(
            "INS/TEST/0001",
            {
                "firstName": "Nadia",
                "lastName": "Insite",
                "email": "nadia.insite@example.com",
        ...
        self.assertFalse(person.matricule_institutionnel, "a submitted matricule is a lookup key, never written")
        self.assertEqual(submission.candidature_id.person_id, person)
        self.assertEqual(submission.candidature_id.state, "prospect")
        self.assertEqual(self.Person.search_count([("email_personnel", "=", "nadia.insite@example.com")]), 1)
        self.assertEqual(person.insite_candidature_count, 1)

    def test_scenario_c_campus_teacher_enters_insite(self):
        applicant = self.env["hr.applicant"].create(
            {
                "partner_name": "Sami Existing",
                "email_from": "sami.existing@example.com",
        ...
        self.assertEqual(submission.person_id, applicant.his_person_id, "one Person, shared with Campus+")
        self.assertEqual(submission.person_id.insite_candidature_count, 1)

    def test_scenario_d_same_teacher_both_processes(self):
        applicant = self.env["hr.applicant"].create(
            {
                "partner_name": "Yacine Both",
                "email_from": "yacine.both@example.com",
    ...
    def test_scenario_e_probabilistic_match_requires_confirmation(self):
        existing = self._person(
            "Ahmed Ben Ali", email_personnel="ahmed.benali@example.com", is_internal_teacher="external"
        )
        submission = self._make_submission(
            "INS/TEST/0003",
            {
        ...
        self.assertEqual(
            self.Person.search_count([("name", "=", "Ahmed Ben Ali")]),
            2,
    ...
    def test_a_mistyped_matricule_does_not_bypass_matching(self):
        self._person("Lina Hadj", email_personnel="lina.hadj@example.com", is_internal_teacher="external")
        submission = self._make_submission(
            "INS/TEST/0004",
            {
                "firstName": "Lina",
                "lastName": "Hadj",
                "email": "lina.hadj@example.com",
                "matricule": "HIS-2099-999999-0",
            },
        )
        submission.with_user(self.manager).action_process()
        self.assertEqual(
            submission.state, "needs_matching", "an unknown matricule must not short-circuit into a new Person"
        )

    def test_matricule_of_an_employee_is_reused(self):
        employee = self._person("Rachid Staff", type_personne="employe", is_internal_teacher="internal")
    ...
    def test_insite_reaches_the_person_of_a_selected_campus_applicant(self):
        applicant = self.env["hr.applicant"].create(
            {
                "partner_name": "Houda Selected",
                "email_from": "houda.selected@example.com",
            }
        )
        person = self._bridge(applicant)
        person.is_internal_teacher = "external"
        submission = self._make_submission(
            "INS/TEST/0006",
            {
                "firstName": "Houda",
                "lastName": "Selected",
                "email": "houda.selected@example.com",
            },
        )
        submission.with_user(self.manager).action_process()
        self.assertEqual(submission.state, "needs_matching")

        wizard = self.env["insite.identity.match.wizard"].with_context(default_submission_id=submission.id).create({})
        wizard.action_search()
    ...
    def test_person_can_have_multiple_campus_applications(self):
        applicant_2025 = self.env["hr.applicant"].create(
            {
                "partner_name": "Yasmine Repeat",
                "email_from": "yasmine.repeat@example.com",
            }
        )
        ...
        wizard = self.env["insite.identity.match.wizard"].create(
            {
                "first_name": "Wizard",
                "last_name": "Bypass",
                "email": "wizard.bypass@example.com",
            }
        )
    ...
    def test_submission_action_open_matching_wizard(self):
        ...
        wizard = self.env["insite.identity.match.wizard"].with_context(**action["context"]).create({})
        ...
        engagement = self.Engagement.create(
            {
                ...
                "insite_candidature_id": candidature.id,
                "need_id": need.id,
            }
        )
...
class TestInsiteNeedPipeline(InsiteCommon):
    ...
    def _schedule_meeting(self, candidature, reschedule=False, start=None, location="Room 101"):
        ...
        wizard = (
            self.env["insite.meeting.schedule.wizard"]
            .with_user(self.manager)
            .with_context(default_candidature_id=candidature.id, reschedule=reschedule)
            .create(
                {
                    "meeting_start": start,
                    "meeting_end": start + timedelta(hours=1),
                    "location": location,
                }
            )
        )
    ...
    def test_internal_teacher_path(self):
        ...
        self.assertEqual(teacher.type_personne, "enseignant", "signing the contract makes the person a teacher")
    ...
    def test_external_candidate_ranking_path(self):
        ...
        strong = self._person("Strong Candidate", specialite="Networking", is_internal_teacher="external")
