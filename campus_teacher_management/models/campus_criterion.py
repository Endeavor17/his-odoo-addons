...
class CampusCriterion(models.Model):
    ...
    version_id = fields.Many2one(
        "campus.evaluation.version", "Evaluation Version", required=True, ondelete="cascade", index=True
    )
    ...
    answered_score = fields.Float(
        ...
        default=5.0,
        digits=(16, 4),
        help="Only used by 'Answered' criteria: the score given when the candidate provided any non-empty answer.",
    )
    ...
    weight_percent = fields.Float("Weight %", compute="_compute_weight_percent", digits=(16, 2))

    scale_ids = fields.One2many("campus.criterion.scale", "criterion_id", "Scale", copy=True)
    ...
    def _validate_for_publish(self):
        ...
        for criterion in self:
            if criterion.value_type == "scale" and len(criterion.scale_ids) < 2:
                raise UserError(
                    _(
                        "Criterion %s needs at least two scale lines before the version can be published.",
                        criterion.code,
                    )
                )
            if criterion.value_type in ("count", "number") and not criterion._effective_max_score():
                raise UserError(
                    _("Criterion %s needs a maximum score so its result can be normalized.", criterion.code)
                )
        return True
    ...
    def _check_max_score(self):
        for criterion in self:
            if criterion.value_type != "scale" and criterion.max_score <= 0 and not criterion.use_version_years_cap:
                raise ValidationError(_("Criterion %s must have a maximum score greater than zero.", criterion.code))
