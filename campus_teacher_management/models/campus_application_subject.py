...
class CampusApplicationSubject(models.Model):
    ...
    subject_id = fields.Many2one("campus.subject", "Subject", ondelete="restrict", index="btree_not_null")
    ...
    is_wished = fields.Boolean("Requested", default=True, help="The candidate asked to teach this subject.")
    is_accepted = fields.Boolean("Accepted", default=False, help="Assigned to the candidate after acceptance.")
    ...
    def _check_subject(self):
        for line in self:
            if not line.subject_id and not (line.subject_name or "").strip():
                raise ValidationError(_("A subject line needs either a catalogue subject or a name."))
