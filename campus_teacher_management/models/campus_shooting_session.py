...
class CampusShootingSession(models.Model):
    ...
    program_id = fields.Many2one(
        ...
        ondelete="cascade",
        index=True,
        help="The teacher's own container for their recording sessions. "
    ...
    event_id = fields.Many2one("calendar.event", "Calendar Entry", readonly=True, copy=False, ondelete="set null")
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if not vals.get("program_id") and vals.get("applicant_id"):
                vals["program_id"] = Program._get_or_create_for_applicant(Applicant.browse(vals["applicant_id"])).id
        ...
        if watched:
            ...
            for session in self:
                if any(old_values[session.id][f] != session[f] for f in watched):
                    ...
                    session.applicant_id.message_post(body=_("Shooting session '%s' updated.", session.title))
        return result
