...
class CampusCourseBreakdown(models.Model):
    ...
    review_notes = fields.Text(
        "Review Notes", help="What needs to change. Sent to the candidate when a revision is requested."
    )
    ...
    def _compute_display_name(self):
        for line in self:
            when = (
                line.create_date and fields.Datetime.context_timestamp(line, line.create_date).strftime("%d %b %Y")
            ) or _("New")
    ...
    def action_approve(self):
        ...
        for line in self:
            ...
            line.applicant_id.message_post(
                body=_(
