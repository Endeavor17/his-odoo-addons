...
class HrApplicant(models.Model):
    ...
    campus_phone2 = fields.Char(
        "Secondary Phone",
        help="Not collected by the current web form; kept so the field exists the day the form starts asking for it.",
    )
    ...
    campus_available_days_display = fields.Char("Campus+ Availability", compute="_compute_multi_displays", store=True)

    campus_teach_methods = fields.Json("Teaching Methods")
    campus_teach_methods_display = fields.Char("Teaching Methods Label", compute="_compute_multi_displays", store=True)

    campus_digital_tools = fields.Json("Digital Tools")
    campus_digital_tools_display = fields.Char("Digital Tools Label", compute="_compute_multi_displays", store=True)
    ...
    campus_subject_ids = fields.One2many("campus.application.subject", "applicant_id", "Subjects")
    ...
    campus_accepted_subject_ids = fields.Many2many(
        "campus.subject", string="Accepted Subjects", compute="_compute_accepted_subjects", store=False
    )
    ...
    campus_version_id = fields.Many2one(
        "campus.evaluation.version", "Evaluation Version", index=True, ondelete="restrict", tracking=True
    )
    campus_score_ids = fields.One2many("campus.application.score", "applicant_id", "Score Breakdown")
    ...
    campus_locked = fields.Boolean(
        "Lock Evaluation", copy=False, help="Protects this application from bulk recalculation."
    )
    campus_external_ref = fields.Char(
        "External Reference",
        index=True,
        copy=False,
        help="Idempotency key from the web form; prevents a retried submission from creating a second application.",
    )
    campus_submission_ids = fields.One2many("campus.submission", "applicant_id", "Raw Submissions")
    ...
    campus_slot_ids = fields.One2many("campus.interview.slot", "applicant_id", "Interview Slots", readonly=True)
    ...
    campus_visible_stage_ids = fields.Many2many(
        ...
        compute="_compute_campus_visible_stage_ids",
        help="Stages offered in the top stage bar for this applicant's job.",
    )

    campus_cb_ids = fields.One2many("campus.course.breakdown", "applicant_id", "Course Breakdown Submissions")
    campus_cb_state = fields.Selection(
        ...
        string="Course Breakdown Status",
        compute="_compute_campus_cb_state",
        store=True,
    ...
    campus_in_contract_stage = fields.Boolean(
        "In Contrat Stage",
        compute="_compute_campus_in_contract_stage",
    ...
    campus_in_cb_stage = fields.Boolean(
        "In Course Breakdown Stage",
        compute="_compute_campus_in_cb_stage",
    ...
    campus_shooting_ids = fields.One2many("campus.shooting.session", "applicant_id", "Shooting Sessions")
    campus_shooting_program_id = fields.Many2one(
        "campus.shooting.program",
        "Shooting Program",
        compute="_compute_campus_shooting_program_id",
    ...
    campus_process_id = fields.Many2one(
        "campus.process",
        "Campus+ Process",
        compute="_compute_campus_process_id",
        store=True,
        index=True,
    ...
    def _compute_accepted_subjects(self):
        for applicant in self:
            applicant.campus_accepted_subject_ids = applicant.campus_subject_ids.filtered(
                lambda line: line.is_accepted and line.subject_id
            ).mapped("subject_id")
    ...
    def _compute_campus_cb_state(self):
        ...
        for applicant in self:
            ...
            elif applicant.campus_hiring_state in ("meeting_2", "interview2_completed", "subjects_selected", "hired"):
    ...
    def _compute_campus_shooting_program_id(self):
        ...
        for applicant in self:
            applicant.campus_shooting_program_id = Program.search([("applicant_id", "=", applicant.id)], limit=1)
    ...
    def _campus_answer_payload(self):
        ...
        return {
            ...
            "hisSubjects": [line.display_subject for line in self.campus_subject_ids if line.source == "free"],
        }
    ...
    def _campus_payload_values(self, payload):
        ...
        if payload.get("nameLat") or payload.get("nameAr"):
            values["partner_name"] = self._campus_clean_scalar(payload.get("nameLat") or payload.get("nameAr"))
        if "email" in payload:
    ...
    def _campus_sync_subjects(self, payload):
        ...
        previously_accepted = {
            ...
            for line in self.campus_subject_ids
            if line.is_accepted
        }
        ...
        for values in lines:
            key = (values.get("subject_id") or False, (values.get("subject_name") or "").strip().lower())
    ...
    def _campus_subject_line(self, item, Subject, source="catalogue"):
        ...
        if isinstance(item, dict):
            if item.get("id"):
                subject = (
                    Subject.search([("code", "=", str(item["id"]))], limit=1)
    ...
    def _campus_scoring_engine(self):
        ...
        dashboard changes.
        """
        name = self.env["ir.config_parameter"].sudo().get_param("campus_teacher.scoring_engine", DEFAULT_ENGINE)
        if name not in self.env:
            _logger.warning("Configured scoring engine %r is not installed; falling back to %s.", name, DEFAULT_ENGINE)
            name = DEFAULT_ENGINE
    ...
    def action_campus_evaluate(self):
        ...
        if not candidates:
            if skipped:
                raise UserError(
                    _("Nothing to evaluate: the selected applications are either locked or have no evaluation version.")
                )
            return False
            ...
            if applicant.campus_hiring_state not in allowed:
                raise UserError(
                    _(
                        ...
                        current=dict(self._fields["campus_hiring_state"].selection).get(applicant.campus_hiring_state),
                        expected=", ".join(
                            dict(self._fields["campus_hiring_state"].selection)[step] for step in allowed
                        ),
                    )
                )
    ...
    def action_campus_select(self):
        ...
        for applicant in self:
            applicant._campus_send_template("campus_teacher_management.mail_template_campus_selected")
            applicant.campus_hiring_state = "invited"
            applicant.message_post(body=_("Selected. Acceptance sent with the available first-meeting times."))
        return True
    ...
    def action_campus_send_documents(self):
        ...
        for applicant in self:
            ...
            applicant._campus_send_template(
                ...
                attachments=[
                    (applicant.campus_cb_filename or "course-breakdown.pdf", applicant.campus_cb_file),
                    (applicant.campus_contract_filename or "contract.pdf", applicant.campus_contract_file),
                ],
            )
    ...
    def action_campus_record_acceptance(self):
        ...
        for applicant in self:
            ...
            applicant.message_post(body=_("Candidate accepted the Course Breakdown and contract by email."))
        return True
    ...
    def action_campus_send_final(self):
        ...
        for applicant in self:
            if not applicant.campus_cb_final_file:
                raise UserError(_("Attach the final Course Breakdown on the Hiring tab before sending."))
            applicant._campus_send_template(
                "campus_teacher_management.mail_template_campus_final",
                attachments=[
                    (applicant.campus_cb_final_filename or "course-breakdown-final.pdf", applicant.campus_cb_final_file)
                ],
            )
            applicant.campus_hiring_state = "final_sent"
            applicant.message_post(body=_("Final Course Breakdown sent with the available second-meeting times."))
        return True
    ...
    def _campus_book_direct_interview(self, round_number, start_datetime, duration=0.5, interviewer=None):
        ...
        existing = self.campus_slot_ids.filtered(lambda s: s.round == round_str and s.state == "booked")[:1]
        if existing:
        ...
        template = (
            "campus_teacher_management.mail_template_campus_interview1_invite"
            if round_str == "1"
            else "campus_teacher_management.mail_template_campus_interview2_invite"
        )
    ...
    def action_campus_mark_interview1_completed(self):
        ...
        self.message_post(body=_("1st interview marked completed. Moved to the 2ème Interview stage."))
    ...
    def action_campus_mark_subjects_selected(self):
        ...
        for applicant in self:
            if not applicant.campus_accepted_subject_ids:
                raise UserError(
                    _("Accept at least one subject on the Subjects tab before marking subject selection complete.")
                )
    ...
    def action_campus_mark_contract_signed(self):
        ...
        for applicant in self:
            if not applicant.campus_contract_file:
                raise UserError(_("Attach the signed Contract PDF on the Hiring tab before marking it signed."))
    ...
    def _campus_latest_cb(self):
        self.ensure_one()
        return self.env["campus.course.breakdown"].search([("applicant_id", "=", self.id)], order="id desc", limit=1)
