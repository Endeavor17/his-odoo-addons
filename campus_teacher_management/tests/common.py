...
class CampusCommon(common.TransactionCase):
    ...
    @classmethod
    def _make_criterion(cls, code, priority, value_type="scale", source_key=None, scale=None, version=None, **extra):
        criterion = cls.Criterion.create(
            dict(
                {
    ...
    def _make_full_criteria_set(cls, version=None):
        ...
        criteria["C2"] = cls._make_criterion("C2", 1, "scale", "taughtCampus", version=version, scale=yes_no)
        criteria["C3"] = cls._make_criterion(
            "C3", 2, "number", "yearsExp", version=version, use_version_years_cap=True, max_score=20
        )
        criteria["C4"] = cls._make_criterion(
            "C4", 1, "scale", "camConfidence", version=version, scale=[("4", 5), ("3", 4), ("2", 3), ("1", 1), ("0", 0)]
        )
        criteria["C5"] = cls._make_criterion(
            "C5", 3, "scale", "taughtSelectedSubjects", version=version, scale=[("oui", 5), ("non", 0)]
        )
        criteria["C6"] = cls._make_criterion("C6", 0, "scale", "taughtHIS", version=version, scale=yes_no)
        criteria["C7"] = cls._make_criterion("C7", 1, "count", "digitalTools", version=version, max_score=6)
        ...
        criteria["C9"] = cls._make_criterion(
            "C9", 1, "scale", "subjectExperienceProvided", version=version, scale=[("oui", 5), ("non", 0)]
        )
        criteria["C10"] = cls._make_criterion("C10", 0, "count", "teachMethods", version=version, max_score=5)
        criteria["C11"] = cls._make_criterion(
            "C11", 1, "answered", "concernsHandled", version=version, answered_score=5
        )
        criteria["C12"] = cls._make_criterion("C12", 2, "answered", "flippedDef", version=version, answered_score=5)
        return criteria

    @classmethod
    def _make_applicant(cls, name="Test Candidate", version=None, **values):
        return cls.Applicant.create(
            dict(
                {
                    "partner_name": name,
