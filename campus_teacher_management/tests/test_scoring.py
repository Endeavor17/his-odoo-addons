...
class TestScoring(CampusCommon):
    ...
    def test_scale_lookup_by_code_label_and_alias(self):
        ...
        self.assertEqual(
            engine._score_scale(criterion, "nonsense")[0], 0.0, "an unmatched answer scores zero rather than raising"
        )
    ...
    def test_count_accepts_lists_and_joined_strings(self):
        ...
        self.assertEqual(
            engine._score_count(criterion, "a، b، c")[0], 3.0, "the original form joined with an Arabic comma"
        )
    ...
    def test_years_saturate_at_the_cap(self):
        ...
        self.assertEqual(
            capped.campus_final_score,
            exactly.campus_final_score,
    ...
    def test_unnormalized_reproduces_a_raw_weighted_sum(self):
        ...
        self.assertEqual(lines["C3"].normalized_score, 30.0, "un-normalized keeps the raw value, cap and all")
        expected = sum(l.raw_score * l.weight_snapshot for l in applicant.campus_score_ids)
        self.assertAlmostEqual(
            applicant.campus_final_score, expected, places=3, msg="no x100 rescale when normalization is off"
        )
    ...
    def test_engine_is_selected_by_config_parameter(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param("campus_teacher.scoring_engine", "campus.scoring.engine")
        self.assertEqual(self.Applicant._campus_scoring_engine()._name, "campus.scoring.engine")

        # An unknown model must fall back rather than crash the evaluation.
        self.env["ir.config_parameter"].sudo().set_param("campus_teacher.scoring_engine", "does.not.exist")
        self.assertEqual(self.Applicant._campus_scoring_engine()._name, "campus.scoring.engine")
