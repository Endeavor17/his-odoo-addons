...
class TestHiringFlow(CampusCommon):
    ...
    def _slot(self, days_ahead=1, hour=9, round_number="1"):
        start = fields.Datetime.now().replace(hour=hour, minute=0, second=0, microsecond=0) + timedelta(days=days_ahead)
    ...
    def test_free_slots_excludes_booked_and_past(self):
        ...
        self.assertNotIn(future, self.Slot._free_slots(1), "a booked slot must not be offered again")
    ...
    def test_candidate_is_not_invited_to_their_own_interview(self):
        ...
        """
        self.applicant.partner_id = (
            self.env["res.partner"].create({"name": "Hire Me", "email": "hire.me@example.com"}).id
        )
        slot = self._slot()
        event = slot._book(self.applicant)
        ...
        self.assertNotIn(
            self.applicant.partner_id, attendees, "the candidate must not be an attendee — Odoo emails attendees"
        )
        self.assertIn(self.env.user.partner_id, attendees, "the interviewer should be")
    ...
    def test_generating_twice_skips_what_already_exists(self):
        ...
        self.env["campus.slot.generate"].create(values).action_generate()
        self.assertEqual(self.Slot.search_count([]), after_first, "running the wizard again must not duplicate slots")
    ...
    def test_full_sequence(self):
        ...
        self.applicant.write(
            {
                "campus_cb_file": b"Y2I=",
                "campus_cb_filename": "cb.pdf",
                "campus_contract_file": b"Y3Q=",
                "campus_contract_filename": "contract.pdf",
            }
        )
        ...
        self.applicant.write({"campus_cb_final_file": b"ZmluYWw=", "campus_cb_final_filename": "final.pdf"})
        ...
        self.assertTrue(
            self.env["hr.employee"].search_count([("name", "=", self.applicant.partner_name)]),
    ...
    def test_emails_are_queued_for_each_send(self):
        ...
        self.applicant.action_campus_select()
        self.assertEqual(Mail.search_count([]) - before, 1, "selecting should queue exactly one email")
