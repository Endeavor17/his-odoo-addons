...
class TestContractStageGate(CampusCommon):
    ...
    def setUp(self):
        ...
        self.manager = self.env["res.users"].create(
            {
                ...
                "group_ids": [(6, 0, [self.env.ref("campus_teacher_management.group_campus_manager").id])],
            }
        )
        self.env["campus.process.permission"].create(
            {
                "process_id": self.process_contract.id,
        ...
        self.applicant = self._make_applicant(
            "Contract Gate Candidate",
            stage_id=self.stage_course_breakdown.id,
            campus_contract_state="sent",
            campus_contract_file=b"Y29udHJhY3Q=",
    ...
    def test_signed_is_blocked_while_at_course_breakdown(self):
        ...
        self.assertEqual(
            self.applicant.campus_contract_state, "sent", "a blocked action must not change the contract status"
        )
