...
class TestCarWeights(CampusCommon):
    ...
    def test_legacy_matches_reference_table(self):
        ...
        for code, expected in LEGACY_EXPECTED.items():
            self.assertAlmostEqual(
                weights[code], expected, places=10, msg=f"legacy_car weight for {code} drifted from the reference table"
            )
    ...
    def test_car_matches_reference_table(self):
        ...
        for code, expected in CAR_EXPECTED.items():
            self.assertAlmostEqual(
                weights[code], expected, places=10, msg=f"car weight for {code} drifted from the reference table"
            )

    def test_both_methods_sum_to_one(self):
        for method in ("car", "legacy_car"):
            self.assertAlmostEqual(
                sum(self._weights(method).values()), 1.0, places=10, msg=f"{method} weights must sum to 1"
            )
    ...
    def test_car_gives_equal_priorities_equal_weight(self):
        ...
        for priority in set(PRIORITIES.values()):
            ...
            self.assertEqual(
                len(distinct),
                1,
                f"criteria with priority {priority} got different weights: { {code: weights[code] for code in group} }",
            )
        ...
        for (code, priority), (next_code, next_priority) in zip(ordered, ordered[1:]):
            if priority > next_priority:
                self.assertGreater(
                    weights[code],
                    weights[next_code],
                    f"{code} (priority {priority}) should outweigh {next_code} (priority {next_priority})",
                )
    ...
    def test_changing_a_priority_reweights_the_set(self):
        ...
        self.assertGreater(criteria["C6"].weight, before, "raising a criterion's priority must raise its weight")
        self.assertAlmostEqual(self.version.total_weight, 1.0, places=8)
