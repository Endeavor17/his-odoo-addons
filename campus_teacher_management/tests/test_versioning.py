...
class TestVersioning(CampusCommon):
    ...
    def test_new_version_creates_an_editable_copy(self):
        ...
        self.assertEqual(
            len(copy.criterion_ids.filtered(lambda c: c.code == "C1").scale_ids),
            6,
        ...
        self.assertEqual(
            self.version.criterion_ids.filtered(lambda c: c.code == "C1").priority,
            3,
    ...
    def test_old_scores_survive_a_barème_change(self):
        ...
        v2.criterion_ids.filtered(lambda c: c.code == "C1").scale_ids.filtered(lambda s: s.key == "mcb").score = 6
        v2.action_publish()
    ...
    def test_locked_application_is_skipped_by_bulk_evaluation(self):
        ...
        self.assertEqual(other.campus_state, "evaluated", "the unlocked application should have been scored")
        self.assertEqual(
            applicant.campus_final_score, score_before, "a locked application must not be re-scored in bulk"
        )
