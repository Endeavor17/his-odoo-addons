...
class CampusApplicationApi(http.Controller):
    ...
    def create_application(self, **kwargs):
        ...
        if not version:
            submission.write(
                {
                    "state": "rejected",
                    "error_code": "no_published_version",
                    "error_message": "No published evaluation version.",
                }
            )
            return self._error("no_published_version", "Applications are not open at the moment.", 422)
        if version.state != "published":
            ...
            return self._error("version_not_published", "Applications are not open at the moment.", 422)
        ...
        duplicate = (
            ...
            if email
            else Applicant.browse()
        )
        if duplicate:
            submission.write(
                {
                    "state": "duplicate",
                    "applicant_id": duplicate.id,
        ...
        submission.write({"state": "processed", "applicant_id": applicant.id, "version_id": version.id})
        return self._json(
            {
                "success": True,
    ...
    def current_version(self, **kwargs):
        ...
        version = (
            request.env["campus.evaluation.version"]
            .sudo()
            .search([("state", "=", "published")], order="version desc, id desc", limit=1)
        )
        if not version:
