...
class CampusScheduleInterview(models.TransientModel):
    ...
    slot_id = fields.Many2one(
        "campus.interview.slot", "Chosen Time", required=True, domain="[('id', 'in', available_slot_ids)]"
    )
    available_slot_ids = fields.Many2many("campus.interview.slot", compute="_compute_available_slot_ids")

    slot_count = fields.Integer("Available", compute="_compute_available_slot_ids")
    interviewer_id = fields.Many2one(related="slot_id.interviewer_id", string="Interviewer", readonly=True)
    ...
    def action_confirm(self):
        ...
        self.applicant_id.campus_hiring_state = "meeting_1" if self.round == "1" else "meeting_2"
