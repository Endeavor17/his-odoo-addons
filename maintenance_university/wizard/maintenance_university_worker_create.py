...
class MaintenanceUniversityWorkerCreate(models.TransientModel):
    ...
    line_ids = fields.One2many("maintenance.university.worker.create.line", "wizard_id", string="Workers")
        ...
        for line in self.line_ids:
            ...
            if person:
                ...
                if user:
                    ...
                    user = Users.create(self._user_vals(line, password, worker_group, person=person))
            else:
...
class MaintenanceUniversityWorkerCreateLine(models.TransientModel):
    ...
    wizard_id = fields.Many2one("maintenance.university.worker.create", required=True, ondelete="cascade")
    ...
    confirmed_new = fields.Boolean(
        string="New Person",
        help="Tick to confirm this is not the person suggested — a different human who happens to share the name.",
    )
