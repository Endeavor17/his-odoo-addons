...
class MaintenanceUniversityWorkday(models.Model):
    ...
    date_end = fields.Datetime(
        string="Ended At",
        help="Empty while the day is still running.",
    )
    segment_ids = fields.One2many(
        "maintenance.university.workday.segment",
        "workday_id",
        string="Segments",
    )
    ...
    def _open_segment(self, kind):
        ...
        return (
            self.env["maintenance.university.workday.segment"]
            .sudo()
            .create(
                {
                    "workday_id": self.id,
...
class MaintenanceUniversityWorkdaySegment(models.Model):
    ...
    employee_id = fields.Many2one(
        related="workday_id.employee_id",
        string="Worker",
        store=True,
        index=True,
    )
