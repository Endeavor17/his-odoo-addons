...
class MaintenanceUniversityFinding(models.Model):
    ...
    image_ids = fields.Many2many(
        "ir.attachment",
        "maintenance_university_finding_image_rel",
        "finding_id",
        "attachment_id",
        string="Photos",
    )
    ...
    def action_submit(self):
        for rec in self:
            ...
            if self.env.user != rec.create_uid and not self.env.user.has_group(
                "maintenance_university.group_maintenance_manager"
            ):
