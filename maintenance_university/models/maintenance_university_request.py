...
class MaintenanceRequest(models.Model):
    ...
    scheduled_date = fields.Datetime(
        string="Scheduled For", tracking=True, help="When the worker is expected to do this."
    )
        ...
        for rec in self:
            ...
            if rec.state not in ("new", "cancelled") and not rec.employee_ids:
                raise ValidationError(
                    _("At least one worker must be assigned before the request leaves the New state.")
                )
