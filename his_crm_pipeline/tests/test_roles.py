...
class TestRoles(TransactionCase):
    ...
    def _demande(
        self, statut_copy="a_faire", statut_design="a_faire", assignee_copy=False, assignee_design=False, **vals
    ):
        ...
        return self.env["crm.lead"].create(
            {
                ...
                "deliverable_ids": [
                    (0, 0, {"type_id": self.type_copy.id, "statut": statut_copy, "assignee_id": assignee_copy}),
                    (0, 0, {"type_id": self.type_design.id, "statut": statut_design, "assignee_id": assignee_design}),
                ],
                **vals,
            }
        )
    ...
    def _livrable(self, demande, type_livrable):
        return demande.deliverable_ids.filtered(lambda d: d.type_id == type_livrable)
    ...
    def test_un_role_admissions_ne_voit_aucune_demande_de_contenu(self):
        ...
        self.assertNotIn(
            demande,
            self.env["crm.lead"].with_user(conseillere).search([]),
        )
    ...
    def test_un_livrable_n_avance_que_par_son_assigne(self):
        ...
        demande = self._demande(
            assignee_copy=redacteur.id,
            assignee_design=graphiste.id,
        )
    ...
    def test_la_priorisation_arbitre_tous_les_livrables(self):
        ...
        self._livrable(demande, self.type_design).with_user(strategiste).assignee_id = strategiste.id
    ...
    def test_la_production_n_affecte_pas_les_livrables(self):
        ...
        with self.assertRaises(AccessError):
            self._livrable(demande, self.type_design).with_user(graphiste).assignee_id = graphiste.id
    ...
    def test_le_demandeur_lit_les_etiquettes(self):
        ...
        self.assertEqual(
            demande.with_user(rh).tag_ids.mapped("name"),
            ["Urgent"],
        )
    ...
    def test_le_responsable_garde_l_analyse(self):
        ...
        self.assertIn(
            self.env.ref("his_crm_pipeline.menu_admissions_cockpit"),
            visibles,
