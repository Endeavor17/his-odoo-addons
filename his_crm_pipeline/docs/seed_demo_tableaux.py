...
def poser(
    nom, etape, owner, tel, src, tags=(), visite=False, tentatives=0, notes=None, equipe=None, motif=None, extra=None
):
    ...
    """
    if Lead.search_count([("name", "=", nom)]) or Lead.with_context(active_test=False).search_count(
        [("name", "=", nom)]
    ):
        return Lead.browse()
    lead = Lead.create(
...
adm = [
    ...
    (
        "Lina Hamadi",
        "stage_vente_pris_en_charge",
        (14.2, 13.5, 12.0),
        ["tag_relance_prioritaire"],
        False,
        aicha,
        "0770110002",
        "Instagram",
    ...
    ("Rania Bouzid", "stage_vente_pre_admis", (10.6, 9.0, 8.5), [], True, rahma, "0770110006", "Bouche a oreille", 0),
]
...
for i, (nom, etape, motif, tent, src) in enumerate(perdues):
    poser(
        nom,
        etape,
        aicha if i % 2 else rahma,
        "0770220%03d" % i,
        src,
        tentatives=tent,
        motif=motif,
        notes=NOTES(12.0 + i, 11.0, 10.0, i),
    )
...
con = [
    ...
    (
        ...
        "marketing",
        "his",
        "stage_contenu_demande",
        [(copy, "a_faire", None), (dsg, "a_faire", None)],
        ["tag_urgent"],
        False,
    ),
    (
        "Video temoignage master",
        "pedagogie",
        "htc",
        "stage_contenu_production",
        [(vid, "en_cours", video_u), (copy, "approuve", redac)],
        ["tag_reseaux_sociaux"],
        hier,
    ),
    (
        "Refonte page filieres",
        "marketing",
        "ira",
        "stage_contenu_production",
        [(copy, "approuve", redac), (dsg, "approuve", design_u)],
        ["tag_site_web"],
        False,
    ),
    (
        "Affiche journee portes ouvertes",
        "hr",
        "his",
        "stage_contenu_approbation",
        [(dsg, "approuve", design_u)],
        ["tag_evenement"],
        False,
    ),
]
    ...
    lead = Lead.create(
        {
            ...
            "tag_ids": [(6, 0, [X(t).id for t in tags])],
            "deliverable_ids": [
                (0, 0, {"type_id": t.id, "statut": s, "assignee_id": a.id if a else False}) for t, s, a in livrables
            ],
        }
    )
...
env.flush_all()
env.cr.execute("UPDATE crm_lead SET date_last_stage_update = now() - interval '9 hours' WHERE name = 'Lina Hamadi'")
env.cr.execute("UPDATE crm_lead SET date_last_stage_update = now() - interval '12 days' WHERE name = 'Rania Bouzid'")
env.cr.commit()
env.invalidate_all()
...
perdues_ids = Lead.with_context(active_test=False).search([("team_id", "=", ventes.id), ("active", "=", False)])
print("\n=== JEU DE DEMONSTRATION ===")
print("Admissions actives :", len(actives))
print("Admissions perdues :", len(perdues_ids), "->", ", ".join(sorted(set(perdues_ids.mapped("lost_reason_id.name")))))
...
print(
    "Fiches personne    :",
    env["his.person"].sudo().search_count([("type_personne", "=", "candidat")]),
...
for lead in actives.filtered("tentatives_appel").sorted("tentatives_appel"):
    print(
        "  %-20s %s tentative(s)%s"
        % (lead.contact_name, lead.tentatives_appel, "  <- pastille ROUGE" if lead.tentatives_appel >= 3 else "")
    )
print(
...
print("\nVues enregistrees :")
for x in (
    "filter_admissions_sla_retard",
    "filter_admissions_candidatures_chaudes",
    "filter_admissions_pre_admis_sans_encaissement",
    "filter_admissions_visite_a_programmer",
    "filter_contenu_livrables_en_retard",
    "filter_contenu_livrables_non_assignes",
    "filter_contenu_attente_approbation",
    "filter_contenu_urgent",
):
    f = X(x)
